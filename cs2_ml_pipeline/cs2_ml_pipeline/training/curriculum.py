...
Phases: Freezetime → Buy → Midplay → Endgame → Full rounds.
"""
...
class CurriculumScheduler:
    ...
    def __init__(self, total_steps: int = 10000, seq_len_start: int = 64,
                 seq_len_end: int = 1024, warmup_steps: int = 1000):
    ...
    def get_phase(self, step: Optional[int] = None) -> CurriculumPhase:
        ...
        if progress < 0.15:    return CurriculumPhase.FREEZETIME
        elif progress < 0.30:  return CurriculumPhase.BUY
        elif progress < 0.70:  return CurriculumPhase.MIDPLAY
        elif progress < 0.90:  return CurriculumPhase.ENDGAME
        else:                  return CurriculumPhase.FULL

    def get_seq_len(self, step: Optional[int] = None) -> int:
        s = step if step is not None else self.current_step
        progress = min(s / max(self.total_steps, 1), 1.0)
        return self.seq_len_start + int((self.seq_len_end - self.seq_len_start) * progress)

    def get_loss_weights(self, step: Optional[int] = None) -> Dict[str, float]:
        phase = self.get_phase(step)
        base = {"next_token": 1.0, "death": 0.1, "value": 0.1, "contrastive": 0.0}
