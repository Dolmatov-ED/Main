...
Ensures data integrity before feeding into tokenization.
"""
...
class DataValidator:
    ...
    def _check_monotonic_ticks(self, df: pd.DataFrame, fpath: Path) -> None:
        ...
        if (diffs < 0).any():
            self.errors.append(f"{fpath.name}: ticks are not monotonically increasing")
