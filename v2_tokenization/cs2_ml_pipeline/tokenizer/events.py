...
Vocabulary includes NULL=0 as padding for ticks with no events.
Composite events (e.g., FLASH + KILL in same tick) get their own IDs
...
EVENT_VOCAB: Dict[str, int] = {
    ...
    "DEATH": 2,
    "DAMAGE": 3,
    "FLASH_AND_KILL": 4,      # Composite: player flashed then killed
    # Bomb
    "PLANT_START": 5,
    "PLANT_ABORT": 6,
    "DEFUSE_START": 7,
...
VOCAB_SIZE = max(EVENT_VOCAB.values()) + 1  # 30

# Priority mapping: when multiple events in same tick, choose highest priority
...
class EventEmbedder(nn.Module):
    ...
    Maps integer event IDs → d_model embeddings.
    """
    ...
    def forward(self, event_ids: torch.Tensor) -> torch.Tensor:
        """
        Args:
            event_ids: [B, S] or [S] — integer event IDs
        Returns:
    ...
    def resolve_event_id(event_names: List[str]) -> int:
        ...
        Uses priority for conflicts. Tries composite tokens with
        stripped suffixes (e.g., FLASH_THROW → FLASH).
        """
        if not event_names:
        ...
        if len(event_names) > 1:
            ...
            # Try stripped suffixes: FLASH_THROW → FLASH, HE_THROW → HE
            stripped = []
